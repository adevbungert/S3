...
const backend = {
    ...
    */
    verifySignatureV2: (stringToSign, signatureFromRequest,
        ...
        var str = JSON.parse(options.requestContext);
        var api_method = str.apiMethod;
        var bucket_name = str.generalResource;
        // var access_test = 'wgfapChmwQ';
        // var secret_test = 'CQx5hm76WuYCDTrHi9y68jVdF7_1Wg';
        const url = "http://localhost:3000/users/" + accessKey + "/authorisation?secret=" + secretKey + "&bucket_name=" + bucket_name + "&api_method=" + api_method;
        console.log(url);
        const req_opt = {
            uri: url,
            method: "GET"
        };
        request(req_opt, function(error, response, body) {
            const res = JSON.parse(body);
            console.log("RES = " + res.code);
            if (res.code == "200") {
                return callback(null, vaultReturnObject);
            } else if (res.code == "403"){
                return callback(errors.PermissionDenied);
